...
exports.createQRCode = async (req, res) => {
  try {
    ...
    const qr = await QRCode.findByPk(Number(id));

    if (!qr) {
      console.log(`❌ QRCode with ID ${id} not found`);
      return res.status(404).json({ error: "QRCode not found" });
    }

    qr.image = cloudinaryResult.secure_url;
    await qr.save();

    console.log("✅ QRCode updated:", qr.toJSON());
    return res.status(200).json({
